...
class wysiwyg
{
	...
	// Drop the promotion link in toolbar! https://stackoverflow.com/questions/74271261/remove-upgrade-button-in-tinymce
	...
	function tinymce_lang()
	{
		...
		$jsFile = e_PLUGIN . 'tinymce5/langs/' . $tinylang[$lang] . '.js';
	...
	function getExternalPlugins($data)
	{
		...
		foreach($tmp as $val)
		{
			$ext[$val] = e_PLUGIN_ABS."tinymce5/plugins/".$val."/plugin.js";
		}
	...
	//	$fl = e107::getFile();
		...
		$configPath = (is_readable(THEME."templates/tinymce/".$template)) ? THEME."templates/tinymce/".$template : e_PLUGIN."tinymce5/templates/".$template;
		...
		//	'editor_selector'   => 'advancedEditor',
			...
			'language_url'      => SITEURLBASE.e_PLUGIN_ABS."tinymce5/langs/" . $tinyMceLanguage . ".js"
		);

	//	if(e_ADMIN_AREA)
		{
	//		$ret['skin_url']     = e_PLUGIN_ABS.'tinymce5/skins/eskin';
		}
...
*/
	...
	//	$ret['skin']                    = 'e107admin';
	//	$ret['skin_url']                = SITEURLBASE.e_PLUGIN_ABS.'tinymce5/skins/e107admin';
...
/*
		$editorCSS = array(
			...
			e_PLUGIN_ABS.'tinymce5/editor.css',
		);
*/
...
/*
	...
	{
	//	$templatePath = (is_readable(THEME."templates/tinymce/".$template)) ? THEME."templates/tinymce/".$template : e_PLUGIN."tinymce5/templates/".$template;
	...
	private function getSnippets()
	{
		...
		{
			$path = e_PLUGIN."tinymce5/snippets";
			$base = e_PLUGIN_ABS."tinymce5/snippets";
		}
		...
		foreach($files as $f)
		{
			...
			if(!empty($m[1]))
			{
			//	$url = e_PLUGIN_ABS."tinymce5/snippets/".$f['fname'];
	...
	public function getEditorCSS($theme = 'front')
	{
		...
		$useThemeStyle = (bool) e107::pref('tinymce5', 'use_theme_style', false);
		...
		$ret[] = e_PLUGIN_ABS . 'tinymce5/editor.css';
